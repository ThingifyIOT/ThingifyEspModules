...
void LedMatrix::commit()
{
	...
	for (byte deviceNumber = 0; deviceNumber < myNumberOfDevices; deviceNumber++)
	{
		for (int bit = 0; bit < 8; bit++)
		{
			...
			for (int c = 0; c < 8; c++)
			{
				bool bitSet = cols[deviceNumber * 8 + 7 - c] & (1 << bit);
				if (bitSet)
				{
					row |= 1 << c;
				}
			...
			sendByte(deviceNumber, bit + 1, row);

			//sendByte(deviceNumber, i + 1, cols[deviceNumber * 8 + i]);
		}
	}
}
...
void LedMatrix::SetDisplay(int displayNumber, bool onOff)
{
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			setPixel(i + displayNumber * 8, j, onOff);
		}
	}
}
...
void LedMatrix::setPixel(byte x, byte y, bool onOff = true)
{
	bitWrite(cols[x], y, onOff);
}
